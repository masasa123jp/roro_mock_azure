...
// iframe 方式の URL（既存 dify.html の実装を踏襲）【既存実装に準拠】
const DIFY_APP_URL = 'https://udify.app/chat/6zuAJFJ2V2iGPknI';

// script 方式のベースURLとトークン（管理画面の Embedded → Script tag で取得）:contentReference[oaicite:8]{index=8}
...
const LOCAL_CHAT_API = '/api/chat'; // POST で {query, conversationId, userId} を受け取り SSE で返す想定。:contentReference[oaicite:9]{index=9}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (USE_DIFY_EMBED) {
    if (EMBED_METHOD === 'script' && EMBED_TOKEN) {
      // script 方式：右下に“バブル”を生成（config → embed.min.js の順序は必須）:contentReference[oaicite:10]{index=10}
      ...
      // 既定：iframe 方式（公式ドキュメントの基本埋め込み）:contentReference[oaicite:11]{index=11}
